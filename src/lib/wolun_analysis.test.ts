...
describe("월운은 양력 월 근사가 아니라 실제 절입 순간을 따른다", () => {
  const saju = calculateSaju(
    "1990-05-15",
    "14:30",
    "solar",
  ...
  const ipchun = getSolarTermsForYear(2024).find(
    (term) => term.term === "입춘",
  )!;
  const before = new Date(ipchun.timestamp - 60_000);
  const after = new Date(ipchun.timestamp + 60_000);

  it("입춘 직전과 직후의 대표 날짜를 전달하면 축월에서 인월로 바뀐다", () => {
    expect(analyzeWolun(saju, 2024, 2, before).monthBranch).toBe("축");
    expect(analyzeWolun(saju, 2024, 2, after).monthBranch).toBe("인");
  });
});
