...
export function recommendTaekil(
  ...
  // 피해야 할 날짜 (하위 점수)
  const worstCandidates = candidates.filter((c) => c.score < 30).slice(0, 5);
...
function evaluateDateForPurpose(
  ...
  > = {
    ...
    "취업/이직": {
      // 《성력고원》의 월건십이신·상관부임·입계약 기준을 현대의
      // 지원/면접(성), 근로계약(정), 첫 출근(개)에 보수적으로 대응한다.
      goodGods: ["정", "성", "개"],
      badGods: ["건", "만", "평", "파", "수", "폐"],
      bonus: 20,
    },
...
function generateGeneralAdvice(
  ...
  const purposeAdvice: Record<TaekilPurpose, string> = {
    ...
    "취업/이직": "채용 일정과 근로조건을 우선하세요. 지원·면접·계약·첫 출근 중 조정 가능한 일정에 참고하는 것이 좋습니다.",
  ...
  // 달력은 브라우저의 로컬 연·월·일로 그리므로 추천 문구도 같은 기준을 써야 한다.
  // 로컬 자정을 toISOString()으로 바꾸면 KST에서는 UTC 전날로 직렬화되어,
  // 실제 최고점 날짜와 문구의 날짜가 하루 어긋난다.
  const year = bestDate.date.getFullYear();
  const month = String(bestDate.date.getMonth() + 1).padStart(2, "0");
  const day = String(bestDate.date.getDate()).padStart(2, "0");

  return `추천 날짜는 ${year}-${month}-${day} (${bestDate.dayPillar}일·${bestDate.score}점)입니다. ${purposeAdvice[purpose]}`;
}
