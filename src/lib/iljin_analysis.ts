 ...
 * 일진 분석
 */
export function analyzeIljin(date: Date, saju: SajuData): IljinAnalysis {
  // 1. 일주 구하기
  ...
  // 4. 사주와의 관계
  const relationWithSaju = analyzeRelationWithSaju(dayStem, dayBranch, saju);

  // 5. 점수 계산
  const score = calculateDayScore(twelveGods, relationWithSaju);
  ...
  // 9. 세부 운세
  const aspects = analyzeAspects(dayPillar, twelveGods, score);
...
function findLuckyHours(
  ...
  EARTHLY_BRANCHES.forEach((branch, i) => {
    if (
      branch === SIX_HARMONY[dayBranch] ||
      _saju.day.branch === SIX_HARMONY[branch]
    ) {
      hours.push({
...
function analyzeAspects(
  ...
  return {
    ...
    career:
      twelveGods.name === "성" || twelveGods.name === "개"
        ? "진전 가능"
        : "안정 유지",
    money: twelveGods.name === "수" ? "재물운 상승" : "보통",
    health: score >= 60 ? "생활 리듬 양호" : "무리하지 않기",
    relationship: score >= 60 ? "원만함" : "신중히 대응",
    study:
      twelveGods.name === "평" || twelveGods.name === "정"
        ? "집중 가능"
        : "보통",
    travel:
      twelveGods.name === "개" || twelveGods.name === "건"
        ? "이동 길함"
        : "주의",
  };
}
