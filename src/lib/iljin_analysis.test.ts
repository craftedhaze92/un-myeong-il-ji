...
describe("analyzeIljin 십이신살 — 절기 기준 월지 회귀 테스트", () => {
  ...
  it("date.getMonth() 근사와 절기 기준 월지가 실제로 다른 날짜다 (전제 조건)", () => {
    ...
    const preciseMonthBranchIndex =
      (getPreciseSolarTermMonthIndex(date) + 2) % 12;
  ...
  it("십이신살이 date.getMonth() 근사가 아닌 절기 기준 월지로 계산된다", () => {
    ...
    const expectedGodIndex =
      (dayBranchIndex - preciseMonthBranchIndex + 12) % 12;
    ...
    expect(result.twelveGods.description).toBe(
      TWELVE_GODS_INFO[expectedGodName]?.desc,
    );
  });
});
...
describe("일진 점수는 기준일이 검증되지 않은 28수에 좌우되지 않는다", () => {
  const saju = calculateSaju(
    "1990-05-15",
    "12:00",
    "solar",
  ...
  it("같은 십이신·사주 관계인 날은 28수가 달라도 점수가 같다", () => {
    const first = analyzeIljin(new Date(2026, 0, 1), saju);
    const comparable = Array.from({ length: 365 }, (_, offset) =>
      analyzeIljin(new Date(2026, 0, 2 + offset), saju),
    ).find(
      (candidate) =>
        candidate.twelveGods.name === first.twelveGods.name &&
        candidate.relationWithSaju.harmony === first.relationWithSaju.harmony &&
        candidate.relationWithSaju.conflict ===
          first.relationWithSaju.conflict &&
        candidate.constellation.fortune !== first.constellation.fortune,
    );

    expect(comparable).toBeDefined();
    expect(comparable!.score).toBe(first.score);
  });
});
